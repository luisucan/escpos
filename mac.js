/*
OBTENER LAS IMPRESORAS: lpstat -p
*/

const fs = require("fs");
const { exec } = require("child_process");
const Jimp = require("jimp"); // Jimp 0.16.1

/* ================================
   CONFIGURACIÓN IMPRESORA CUPS
================================ */

// Cambia esto por el nombre real (ver con: lpstat -p)
const PRINTER_NAME = "Printer_POS_80";  

/* ================================
   BUFFER GLOBAL ESC/POS
================================ */
let ESC_POS = [];
...
function writeAscii(text) {
  ESC_POS.push(...Buffer.from(text, "ascii"));
}
...
async function printImage(path) {
  ...
  esc([0x1D, 0x76, 0x30, 0x00, width / 8, 0x00, height & 0xff, height >> 8]);
...
function sendToCups() {
  const buffer = Buffer.from(ESC_POS);

  const child = exec(`lp -o raw -d "${PRINTER_NAME}"`, (err) => {
...
async function main() {
  ...
  sendToCups(); // ← enviar ticket completo
}
